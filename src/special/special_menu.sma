...
public plugin_init()
{
    register_plugin("special_menu", "0.2.0", "Ako");
...
{
    new menu[256];
    new enabled[16], mode[32];

    copy(enabled, charsmax(enabled), fake_aimbot_get(id) ? "\y開啟" : "\r關閉");
    copy(mode, charsmax(mode), fake_aimbot_get_mode(id) == AIM_MODE_FIRE ? "開火時" : "持續鎖定");

    formatex(menu, charsmax(menu), g_aimbotBody, enabled, mode);
