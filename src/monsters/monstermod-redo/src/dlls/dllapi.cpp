...
extern cvar_t *monster_dynamic_respawn;
extern cvar_t *monster_dynamic_respawn_delay;
...
monster_type_t monster_types[]=
{
	...
	"monster_hutao", FALSE, // Custom Hutao monster
	"", FALSE
};
...
monster_dynamic_respawn_t monster_dynamic_respawn_queue[MAX_DYNAMIC_RESPAWNS];
bool track_dynamic_respawn_spawn = FALSE;
...
void FreeMonsterIndex(int index)
{
	...
	monsters[index].respawn_enabled = FALSE;
	monsters[index].respawn_monster_type = -1;
...
void ScheduleMonsterRespawn(edict_t *pMonster)
{
	if (!monster_dynamic_respawn->value || !monster_spawn->value)
		return;

	if (FNullEnt(pMonster))
		return;

	for (int index = 0; index < monster_ents_used; index++)
	{
		if (monsters[index].monster_pent != pMonster || !monsters[index].respawn_enabled)
			continue;

		monsters[index].respawn_enabled = FALSE;

		for (int respawn = 0; respawn < MAX_DYNAMIC_RESPAWNS; respawn++)
		{
			if (monster_dynamic_respawn_queue[respawn].active)
				continue;

			float delay = monster_dynamic_respawn_delay->value;
			if (delay < 1.0f)
				delay = 1.0f;

			monster_dynamic_respawn_queue[respawn].active = TRUE;
			monster_dynamic_respawn_queue[respawn].monster_type = monsters[index].respawn_monster_type;
			monster_dynamic_respawn_queue[respawn].origin = monsters[index].respawn_origin;
			monster_dynamic_respawn_queue[respawn].angles = monsters[index].respawn_angles;
			monster_dynamic_respawn_queue[respawn].spawnflags = monsters[index].respawn_spawnflags;
			monster_dynamic_respawn_queue[respawn].respawn_time = gpGlobals->time + delay;
			return;
		}

		LOG_MESSAGE(PLID, "ERROR: can't schedule dynamic monster respawn, reached MAX_DYNAMIC_RESPAWNS!");
		return;
	}
}
...
edict_t* spawn_monster(int monster_type, Vector origin, Vector angles, int spawnflags, pKVD *keyvalue)
{
	...
	switch (monster_type)
	{
		...
		case 35: monsters[monster_index].pMonster = CreateClassPtr((CMHutao *)NULL); break;
	}
	...
	monsters[monster_index].respawn_enabled = track_dynamic_respawn_spawn;
	monsters[monster_index].respawn_monster_type = monster_type;
	monsters[monster_index].respawn_origin = origin;
	monsters[monster_index].respawn_angles = angles;
	monsters[monster_index].respawn_spawnflags = spawnflags;
...
edict_t* spawn_dynamic_respawn_monster(int monster_type, Vector origin, Vector angles, int spawnflags, pKVD *keyvalue)
{
	BOOL previous = track_dynamic_respawn_spawn;
	track_dynamic_respawn_spawn = TRUE;
	edict_t *pent = spawn_monster(monster_type, origin, angles, spawnflags, keyvalue);
	track_dynamic_respawn_spawn = previous;
...
void check_dynamic_respawn(void)
{
	if (!monster_dynamic_respawn->value || !monster_spawn->value)
		return;

	for (int index = 0; index < MAX_DYNAMIC_RESPAWNS; index++)
	{
		if (!monster_dynamic_respawn_queue[index].active || monster_dynamic_respawn_queue[index].respawn_time > gpGlobals->time)
			continue;

		monster_dynamic_respawn_queue[index].active = FALSE;
		spawn_dynamic_respawn_monster(
			monster_dynamic_respawn_queue[index].monster_type,
			monster_dynamic_respawn_queue[index].origin,
			monster_dynamic_respawn_queue[index].angles,
			monster_dynamic_respawn_queue[index].spawnflags,
			NULL
		);
	}
}
			...
			if (player_index != -1)
			{
				...
				if (tr.flFraction >= 1.0)
				{
					...
					if (tr.flFraction < 1.0)	 // hit something?
					{
						...
						spawn_dynamic_respawn_monster(monster_type, v_src, monster_angle, spawnflags, NULL);
...
int mmDispatchSpawn( edict_t *pent )
{
	...
	if (strcmp(pClassname, "worldspawn") == 0)
	{
		...
		for (index = 0; index < MAX_DYNAMIC_RESPAWNS; index++)
			monster_dynamic_respawn_queue[index].active = FALSE;
